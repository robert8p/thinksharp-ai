...
export default function SignInScreen() {
  ...
  return (
    <Screen>
      <FunHeader emoji="🔐" title="Save your progress" subtitle="Create an account for production auth, or skip and explore the demo instantly." />
      <CoachBubble text="For the no-code demo deployment, you can safely choose skip. Supabase can be connected later." emoji="✅" />
      <Card>
      ...
      <Button onPress={handleSubmit(submit)}>Create account</Button>
      <Button variant="secondary" onPress={() => { completeOnboarding(); router.replace('/(tabs)'); }}>Skip for demo</Button>
    </Screen>
  );
}

const styles = StyleSheet.create({
  input: { color: colors.text, borderColor: colors.border, borderWidth: 2, borderRadius: radii.lg, padding: spacing.md, marginBottom: spacing.sm, backgroundColor: colors.white }
});
